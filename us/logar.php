...
?>
<div id="cj"><span class="titulo">Login</span></div>
<div id="fj"><div id="ctj">
<div align="center">
<form method="post" action="<?php $PHP_SELF; ?>">
<table>
<tr><td><span class="texto">Login: </span></td><td><input type="text" name="login" id="input1"></td></tr>
<tr><td><span class="texto">Senha:</span></td><td><input type="password" name="senha" id="input1"></td></tr>
<tr><td><input type="checkbox" name="lembrar" id="input1"><span class="texto">Manter-me conectado</span></td><td>
<button type="submit" name="logar" value="Logar" id="input1">Logar</button></td></tr></table>
</form>
<span class="texto"><?php echo $mensagem_erro; ?></span>
<br>
</div></div>
</div>
<div id="cj" style="margin-top: 4px;"><span class="titulo">Cadastre-se</span></div>
<div id="fj"><div id="ctj">
<?php
...
if(isset($_POST["cadastrar"])) { // Verifico se o botão cadastrar foi acionado
	...
	if(!empty($_POST["login"]) && !empty($_POST["senha"]) && !empty($_POST["email"])) { // Verifico se os campos foram preenchidos
		...
		$repeat_user = mysql_query("SELECT * FROM $tabela WHERE nome='$login'") or die($mensagem_erro = "Houve um erro:<br />".mysql_error()); // Faço a consulta ao SQL para verificar se não há usuários com o mesmo login name
		...
		if(mysql_num_rows($repeat_user) > 0) { // Verifico se a consulta retorna algum resultado. Nesse caso, se retornar, define uma mensagem de erro.
			...
			$insert = mysql_query("INSERT INTO $tabela(login, senha, email, nome, sobrenome, foto, adm) VALUES('$login', '$senha', '$email', '$nome', '$sobrenome', 'novo/novo.jpg', '0')") or die(mysql_error()); // Insiro os dados no BD
...
<table width="220" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><span class="texto"><?php echo $mensagem_erro1; ?></span></td>
  </tr>
  <tr>
    <td><table width="220" border="0" cellspacing="0" cellpadding="5">
      ...
      <tr>
        ...
        <td><label>
          <input id="input1" type="text" name="nome" />
		</label></td>
      </tr>
      <tr>
        <td><span class="texto">Sobrenome:</span></td>
        <td><label>
          <input id="input1" type="text" name="sobrenome" />
		</label></td>
      </tr>
      <tr>
        <td>&nbsp;</td>
